export default {
  // ======================================================
  // Single word translations
  // ======================================================
  home: "الرئيسية",
  title: "عبد العزيز الحضري",
  me: "عبد العزيز الحضري",
  about: "عني",
  projects: "المشاريع",
  contact: "تواصل",
  navigation: "التنقل",
  resources: "المصادر",
  blog: "المدونة",
  resume: "السيرة الذاتية",
  contactMe: "تواصل معي",
  education: "التعليم",
  overview: "نظرة عامة",
  technologies: "التقنيات",
  challenges: "التحديات",
  solutions: "الحلول",
  screenshots: "لقطات الشاشة",
  certifications: "الشهادات",
  name: "الاسم",
  email: "البريد الإلكتروني",
  message: "الرسالة",
  category: "الفئة",

  // ======================================================
  // Multi-word translations
  // ======================================================
  greeting: "مرحباً، أنا ",
  viewProjects: "عرض المشاريع",
  aboutMe: "نبذة عني",
  getInTouch: "تواصل معي",
  pageNotFound: "الصفحة غير موجودة",
  skills: "المهارات والتقنيات",
  experience: "الخبرة العملية",
  viewLive: "عرض مباشر",
  goBack: "رجوع",
  allRightsReserved: "جميع الحقوق محفوظة.",
  viewCode: "عرض الكود",
  viewDetails: "عرض التفاصيل",
  backToProjects: "العودة للمشاريع",
  keyFeatures: "المميزات الرئيسية",
  challengesAndSolutions: "التحديات والحلول",
  searchPosts: "البحث في المقالات",
  noPostsFound: "لم يتم العثور على مقالات.",
  postNotFound: "المقال غير موجود",
  backToBlog: "العودة للمدونة",
  recommendedPosts: "مقالات مقترحة",
  namePlaceholder: "أدخل اسمك",
  emailPlaceholder: "أدخل بريدك الإلكتروني",
  messagePlaceholder: "اكتب رسالتك هنا...",
  send: "إرسال الرسالة",
  backToHome: "العودة للرئيسية",
  educationTitle: "بكالوريوس علوم الحاسب",
  educationDate: "جامعة المنوفية • ٢٠٢٠ - ٢٠٢٤",
  acknowledgment: "شكر وتقدير",
  learnMore: "اقرأ المزيد",
  contactInformation: "معلومات التواصل",
  phone: "الهاتف",
  copiedToClipboard: "تم النسخ",
  copiedToClipboardDescription: "تم نسخ الرابط",
  workTitle: "مطور Full Stack",
  projectNotFound: "المشروع غير موجود",

  // ======================================================
  // Long descriptions
  // ======================================================
  homeDescription:
    "مطور ويب متكامل (MERN) متخصص في Next.js و React Native. شغوف ببناء تطبيقات ويب وموبايل ديناميكية.",
  aboutDescription:
    "أنا مطور ويب متكامل متخصص في MERN stack وNext.js وReact Native. كخريج حديث في علوم الحاسب، أنا شغوف ببناء تطبيقات ويب وموبايل ديناميكية. أحب تحويل الأفكار إلى تجارب سلسة وسهلة الاستخدام مع التعلم المستمر واستكشاف التقنيات الجديدة.",
  footerDescription: "بناء تجارب رائعة من خلال البرمجة والتصميم.",
  educationDescription:
    "درست علوم الحاسب مع التركيز على تقنيات الويب ومبادئ هندسة البرمجيات.",
  projectsDescription:
    "هذه بعض المشاريع التي عملت عليها. كل مشروع تم بناؤه بعناية واهتمام بالتفاصيل.",
  contactDescription: "هل لديك سؤال أو تريد العمل معاً؟ لا تتردد في التواصل!",
  notFoundDescription:
    "عذراً! الصفحة التي تبحث عنها غير موجودة. ربما تم نقلها أو حذفها.",
  blogDescription: "أكتب عن تطوير الويب والتصميم وأشياء أخرى تهمني.",
  projectNotFoundDescription:
    "عذراً، لم نتمكن من العثور على المشروع الذي تبحث عنه. ربما تم نقله أو حذفه.",
} as const;
